...
- [ ] `package.json`에 `zod`를 직접 의존성으로 추가하고 전이 의존성 의존을 제거하기
- [ ] `openapi.json`, `openapi.yaml` 드리프트 원인을 정리하고 현재 코드 기준으로 산출물 동기화하기
- [ ] OpenAPI 산출물 드리프트를 CI에서 자동 검출하도록 검사 단계 추가하기

## P1

- [ ] `ServiceRegistry.initializeAll`, `cleanupAll`을 실제 런타임에 연결하거나 인터페이스에서 제거할지 결정하기
...
- [ ] `workers-mcp`가 실제로 불필요하면 제거하기
- [ ] `package.json` 설명과 일부 문서를 현재 멀티서비스 구조에 맞게 업데이트하기
- [ ] `docs/` 분석 문서를 서비스별 인덱스로 묶어 탐색성을 높이기
