...
- 메가박스 좌석맵 조회: 회차 ID 기준 read-only 좌석 구조와 요금표 조회
...
3. 재고/좌석 확인 요청 시:
   ...
   - 메가박스: 먼저 상영작/회차 조회 후 필요 시 `theaterId`/`movieId`로 잔여 좌석 조회, 회차 ID가 있으면 좌석맵 조회
...
- 메가박스 좌석맵 조회는 `playSchdlNo`가 반드시 필요하며, 좌석 점유/선점 여부까지는 보장하지 않음
